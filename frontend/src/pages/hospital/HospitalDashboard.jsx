...
export default function HospitalDashboard() {
  ...
  const { data: reqData } = useGetHospitalRequestsQuery({ status:"pending", limit:5 });

  const s = statsData?.data || {};
  const pending = reqData?.data || [];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"22px" }}>
      ...
      <Card>
        <h3 style={{ fontFamily:"'Syne',sans-serif", fontSize:"16px", fontWeight:700, marginBottom:"16px" }}>Pending Requests</h3>
        {pending.length === 0
            ...
            <div key={r._id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 0", borderBottom:"1px solid rgba(42,42,48,0.5)", flexWrap:"wrap", gap:"10px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:"12px", color:"#8E8E9A" }}>{new Date(r.createdAt).toLocaleDateString()} • {r.units} unit(s)</p>
                </div>
              </div>
