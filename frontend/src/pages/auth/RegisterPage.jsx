...
export default function RegisterPage() {
  ...
  const [registerHospital, { isLoading: hLoading }] =
    useRegisterHospitalMutation();
  ...
  const handleHospital = async (e) => {
    ...
    try {
      await registerHospital({
        name: hForm.name,
        email: hForm.email,
        password: hForm.password,
        licenseNumber: hForm.licenseNumber,
        contact: { phone: hForm["contact.phone"] },
        location: {
          city: hForm["location.city"],
          address: hForm["location.address"],
        },
      }).unwrap();
    ...
    >
      <div style={{ width: "100%", maxWidth: "460px" }} className="fade-up">
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: "11px",
                borderRadius: "10px",
                border: `1px solid ${tab === t.id ? "#E8192C" : "#2A2A30"}`,
                background: tab === t.id ? "rgba(232,25,44,0.1)" : "#1F1F24",
                color: tab === t.id ? "#FF4D5E" : "#8E8E9A",
          ...
          {tab === "user" ? (
            ...
            >
              ...
              >
                ...
                <Input
                  label="Blood Group"
                  type="select"
                  value={uForm.bloodGroup}
                  onChange={(e) =>
                    setUForm({ ...uForm, bloodGroup: e.target.value })
                  }
                  options={BG.map((g) => ({ value: g, label: g }))}
                />
              </div>
              <Input
                label="Phone"
              ...
              <Input
                label="City"
                value={uForm["location.city"]}
                onChange={(e) =>
                  setUForm({ ...uForm, "location.city": e.target.value })
                }
                placeholder="Mumbai"
                icon="📍"
              />
            ...
            >
              ...
              <Input
                label="Password"
                type="password"
                value={hForm.password}
                onChange={(e) =>
                  setHForm({ ...hForm, password: e.target.value })
                }
                placeholder="Min 6 characters"
                icon="🔒"
                required
              ...
              <Input
                ...
                value={hForm.licenseNumber}
                onChange={(e) =>
                  setHForm({ ...hForm, licenseNumber: e.target.value })
                }
                placeholder="LIC-2041"
                icon="📋"
                required
              />
              <Input
                label="Phone"
                value={hForm["contact.phone"]}
                onChange={(e) =>
                  setHForm({ ...hForm, "contact.phone": e.target.value })
                }
                placeholder="+91 9876543210"
                icon="📞"
              />
              <Input
                label="City"
                value={hForm["location.city"]}
                onChange={(e) =>
                  setHForm({ ...hForm, "location.city": e.target.value })
                }
                placeholder="Mumbai"
                icon="📍"
              />
              <Input
                label="Address"
                value={hForm["location.address"]}
                onChange={(e) =>
                  setHForm({ ...hForm, "location.address": e.target.value })
                }
              ...
              >
                <p style={{ fontSize: "12px", color: "#F59E0B" }}>
                  ⚠️ Hospital registration requires admin approval before you
                  can login.
                </p>
              </div>
              <Button
        ...
        >
          ...
          <Link to="/login" style={{ color: "#E8192C", fontWeight: 600 }}>
            Login
          </Link>
        </p>
      </div>
